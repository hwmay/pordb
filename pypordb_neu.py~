...
class Neueingabe(QtGui.QDialog, pordb_neu):
	def __init__(self, verzeichnis, verzeichnis_original, verzeichnis_thumbs, verzeichnis_trash, verzeichnis_cover, bilddatei, titel=None, darsteller=None, cd=None, bild=None, nurbild=None, original=None, cs=None, vorhanden=None, cover=None, undo=None, cover_anlegen=None, original_weitere=None, original_cover = None, high_definition = None):
		...
		window_size = settings.value("Neueingabe/Size", QtCore.QVariant(QtCore.QSize(600, 500))).toSize()
		self.resize(window_size)
		window_position = settings.value("Neueingabe/Position", QtCore.QVariant(QtCore.QPoint(0, 0))).toPoint()
	...
	def darsteller_sortieren(self, darsteller):
		...
		for i in darsteller:
			if i:
				...
				zu_lesen = "select sex from pordb_darsteller where darsteller = '" + i.strip().title().replace("'", "''") + "'"
				self.lese_func = DBLesen(self, zu_lesen)
				res = DBLesen.get_data(self.lese_func)
				try:
					sex = res[0][0]
	...
	def closeEvent(self, event):
		settings = QtCore.QSettings()
		settings.setValue("Neueingabe/Size", QtCore.QVariant(self.size()))
		settings.setValue("Neueingabe/Position", QtCore.QVariant(self.pos()))
