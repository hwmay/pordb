...
class UpdateVersion(QtGui.QDialog, pordb_update_version):
	def __init__(self, version, whatsnew):
		...
		window_size = settings.value("UpdateVersion/Size", QtCore.QVariant(QtCore.QSize(600, 500))).toSize()
		self.resize(window_size)
		window_position = settings.value("UpdateVersion/Position", QtCore.QVariant(QtCore.QPoint(0, 0))).toPoint()
	...
	def accept(self):
		...
		while True:
			zaehler += 1
			try:
				seite = urllib2.urlopen(file_download)
				if seite:
					break
				else:
					pass
	...
	def closeEvent(self, event):
		settings = QtCore.QSettings()
		settings.setValue("UpdateVersion/Size", QtCore.QVariant(self.size()))
		settings.setValue("UpdateVersion/Position", QtCore.QVariant(self.pos()))
