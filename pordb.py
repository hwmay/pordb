...
class MeinDialog(QtGui.QMainWindow, MainWindow):
	...
	def onOriginal(self):
		...
		self.start_bilder = 0
		try:
			ein = unicode(self.suchfeld.currentText()).replace("'", "''").replace("#","").lower().strip().encode("utf-8")
		except:
			message = QtGui.QMessageBox.critical(self, self.trUtf8("Error "), self.trUtf8(u"Seems to be an invalid character in the search field"))
			return
		if not ein or ein == "=":
	...
	def onDarstellerspeichern(self):
		name = unicode(self.labelDarsteller.text()).replace("'", "''")
		if not name:
			return
		try:
	...
	def onPartnerZeigen(self):
		...
		self.aktuelles_res = []
		for i in xrange(self.listWidgetDarsteller.count()):
			self.aktuelles_res.append(unicode(self.listWidgetDarsteller.item(i).text()).encode("utf-8").strip())
	...
	def GetWebsite(self):
		if unicode(self.lineEditURL.text()).startswith("http://"):
